...
@section('title', 'Edit Task — SocietyMS')

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('task.index') }}">Tasks</a></li>
    <li class="breadcrumb-item"><a href="{{ route('task.show', $task) }}">{{ $task->title }}</a></li>
...
@php
    $isRep       = old('is_repetitive', $task->is_repetitive ? '1' : '0') === '1';
    $recType     = old('recurrence_type', $task->recurrence_type ?? '');
    $recEnds     = old('recurrence_ends', $task->recurrence_ends ?? '');
    $oldWeekDays = old('week_days', $task->week_days ?? []);
    $oldMonths   = old('months',    $task->months    ?? []);
@endphp

<div class="d-flex align-items-center gap-3 mb-4">
    <a href="{{ route('task.show', $task) }}" class="btn btn-sm btn-outline-secondary">
        <i class="bi bi-arrow-left"></i>
    </a>
    <div>
        <h4 class="fw-bold text-dark mb-0">Edit Task</h4>
        <p class="text-muted small mb-0">Update details for {{ $task->title }}</p>
    </div>
</div>
        ...
        <form action="{{ route('task.update', $task) }}" method="POST">
            ...
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body p-4">
                    ...
                    {{-- Non-repetitive --}}
                    <div id="section_deadline" {{ $isRep ? 'style="display:none"' : '' }}>
                        ...
                        <input type="date" id="deadline_date" name="deadline_date"
                               value="{{ old('deadline_date', $task->deadline_date?->format('Y-m-d')) }}"
                    ...
                    {{-- Repetitive --}}
                    <div id="section_repetitive" {{ $isRep ? '' : 'style="display:none"' }}>
                        ...
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" id="days_to_complete" name="days_to_complete"
                                       value="{{ old('days_to_complete', $task->days_to_complete) }}" min="1"
                            ...
                            <div class="col-md-4">
                                ...
                                <select id="recurrence_type" name="recurrence_type"
                                    ...
                                    <option value="weekly"    {{ $recType === 'weekly'    ? 'selected' : '' }}>Weekly (Specific Days)</option>
                                    <option value="monthly"   {{ $recType === 'monthly'   ? 'selected' : '' }}>Monthly (Specific Date)</option>
                                    <option value="quarterly" {{ $recType === 'quarterly' ? 'selected' : '' }}>Quarterly (Every 3 Months)</option>
                                    <option value="biannual"  {{ $recType === 'biannual'  ? 'selected' : '' }}>Biannual (Every 6 Months)</option>
                                    <option value="annual"    {{ $recType === 'annual'    ? 'selected' : '' }}>Annual (Every 12 Months)</option>
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <select id="recurrence_ends" name="recurrence_ends"
                                    ...
                                    <option value="never"             {{ $recEnds === 'never'             ? 'selected' : '' }}>Never Ends</option>
                                    <option value="after_occurrences" {{ $recEnds === 'after_occurrences' ? 'selected' : '' }}>After X Occurrences</option>
                                    <option value="on_date"           {{ $recEnds === 'on_date'           ? 'selected' : '' }}>On Specific Date</option>
                                </select>
                        ...
                        {{-- After X Occurrences --}}
                        <div id="section_occurrences" class="mb-3"
                             {{ $recEnds === 'after_occurrences' ? '' : 'style="display:none"' }}>
                            ...
                            <input type="number" id="occurrences" name="occurrences"
                                   value="{{ old('occurrences', $task->occurrences ?? 1) }}" min="1"
                        ...
                        {{-- On Specific Date --}}
                        <div id="section_end_date" class="mb-3"
                             {{ $recEnds === 'on_date' ? '' : 'style="display:none"' }}>
                        ...
                        <div id="section_week_days" class="mb-3"
                             {{ $recType === 'weekly' ? '' : 'style="display:none"' }}>
                                ...
                                <div class="col-sm-3 col-6">
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input"
                                               ...
                                               {{ in_array($num, $oldWeekDays) ? 'checked' : '' }}>
                        ...
                        <div id="section_monthly" class="mb-3"
                             {{ $recType === 'monthly' ? '' : 'style="display:none"' }}>
                            <div class="mb-3">
                                ...
                                <select id="month_day" name="month_day"
                                    ...
                                    @for($d = 1; $d <= 31; $d++)
                                        <option value="{{ $d }}"
                                            {{ old('month_day', $task->month_day ?? 1) == $d ? 'selected' : '' }}>
                            ...
                            <div>
                                <label class="form-label fw-semibold">
                                    Select months <span class="text-muted fw-normal">(leave blank for all months)</span>
                                </label>
                                <div class="row g-2">
                                    ...
                                    <div class="col-sm-3 col-6">
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input"
                                                   ...
                                                   {{ in_array($num, $oldMonths) ? 'checked' : '' }}>
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('task.show', $task) }}" class="btn btn-outline-secondary px-4">Cancel</a>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    toggleRecurrence(document.getElementById('is_repetitive').checked);
});
</script>
@endpush
