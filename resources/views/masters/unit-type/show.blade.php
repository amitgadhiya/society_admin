...
@section('title', $unitType->name . ' — SocietyMS')
...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h4 class="fw-bold text-dark mb-0">{{ $unitType->name }}</h4>
                <p class="text-muted small mb-0">Unit type details &amp; assigned units</p>
            </div>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('unit-type.edit', $unitType) }}" class="btn btn-warning btn-sm">
                <i class="bi bi-pencil me-1"></i>Edit
            </a>
            <form action="{{ route('unit-type.destroy', $unitType) }}" method="POST"
                  onsubmit="return confirm('Delete unit type \'{{ addslashes($unitType->name) }}\'? This cannot be undone.')">
                @csrf @method('DELETE')
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm">
                <div class="card-header py-3">
                    <i class="bi bi-info-circle me-2"></i>Type Information
                </div>
                <div class="card-body p-4">
                    <div class="row g-4">
                        <div class="col-12">
                            <p class="text-muted small fw-semibold mb-1">Name</p>
                            <p class="fw-semibold mb-0">{{ $unitType->name }}</p>
                        </div>
                        <div class="col-12">
                        ...
                        <div class="col-12">
                            ...
                            <p class="fw-semibold mb-0">{{ optional($unitType->created_at)->format('d M Y') }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-8">
            <div class="card shadow-sm">
                <div class="card-header py-3 d-flex justify-content-between align-items-center">
                    <span><i class="bi bi-house me-2"></i>Units with this type</span>
                ...
                @if ($units->isEmpty())
                    <div class="text-center py-5 text-muted">
                        ...
                        <p class="small mb-0">No units assigned to this type yet.</p>
                    </div>
                @else
                            ...
                            <tbody>
                                @foreach ($units as $unit)
                                    <tr>
                                        ...
                                        <td class="py-3 text-muted small">{{ $unit->wing?->name ?? '—' }}</td>
