...
@section('title', 'Edit Unit — SocietyMS')

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('unit.index') }}">Units</a></li>
    <li class="breadcrumb-item"><a href="{{ route('unit.show', $unit) }}">{{ $unit->unit_number }}</a></li>
...
@section('content')
    ...
    <div class="d-flex align-items-center gap-3 mb-4">
        <a href="{{ route('unit.show', $unit) }}" class="btn btn-sm btn-outline-secondary">
            <i class="bi bi-arrow-left"></i>
        </a>
        <div>
            <h4 class="fw-bold text-dark mb-0">Edit Unit</h4>
            <p class="text-muted small mb-0">Update details for unit {{ $unit->unit_number }}</p>
        </div>
    </div>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('unit.update', $unit) }}" method="POST">
                        ...
                        <div class="row g-3">
                                ...
                                <select id="wing_id" name="wing_id"
                                    ...
                                    @foreach ($wings as $wing)
                                        <option value="{{ $wing->id }}"
                                            {{ old('wing_id', $unit->wing_id) == $wing->id ? 'selected' : '' }}>
                                            {{ $wing->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-sm-6">
                                ...
                                <select id="unit_type_id" name="unit_type_id"
                                    ...
                                    @foreach ($unitTypes as $ut)
                                        <option value="{{ $ut->id }}"
                                            {{ old('unit_type_id', $unit->unit_type_id) == $ut->id ? 'selected' : '' }}>
                                            {{ $ut->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-sm-6">
                                ...
                                <select id="status" name="status"
                                    ...
                                    @foreach (['active' => 'Active', 'inactive' => 'Inactive'] as $val => $label)
                                        <option value="{{ $val }}"
                                            {{ old('status', $unit->status) === $val ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-sm-6">
                                ...
                                <input type="text" id="registered_in_name_of" name="registered_in_name_of"
                                       value="{{ old('registered_in_name_of', $unit->registered_in_name_of) }}"
                            ...
                            <div class="col-sm-6">
                                ...
                                <input type="text" id="contact_number" name="contact_number"
                                       value="{{ old('contact_number', $unit->contact_number) }}"
                            ...
                            <div class="col-sm-6">
                                ...
                                <input type="number" id="opening_balance" name="opening_balance"
                                       value="{{ old('opening_balance', $unit->opening_balance) }}"
                                       placeholder="0.00"
                                       min="0" step="0.01"
                        ...
                        <div class="d-flex gap-2 pt-4 mt-2 border-top">
                            ...
                            <a href="{{ route('unit.show', $unit) }}" class="btn btn-outline-secondary flex-fill">
                                Cancel
                            </a>
                        </div>
                    </form>
